...
export class SyncDocDO extends DurableObject<Env> {
  ...
  private boot(): void {
    ...
    // Set the adapter's peerId SYNCHRONOUSLY, before any join can be handled. The
    // Repo's networkSubsystem also calls net.connect(serverPeerId), but that can land
    // AFTER the first join — and handleJoin announces `senderId: this.peerId`. If it
    // were still undefined, the client would learn an empty server peerId, address
    // every sync frame to targetId:"" (which handleFrame can't route), so the in-DO
    // Repo would never receive the doc → repo.find() times out "unavailable" →
    // nothing persists → stuck "Saving…". (Root-caused on prod: stuck sync frames had
    // target="" + serverPeer=undefined — this is the fix.)
    this.net.peerId = serverPeerId;
